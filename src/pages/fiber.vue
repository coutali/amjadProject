        ...
        <VCardTitle class="d-flex mt-5 mr-5">
          <VRow>
            ...
            <VCol cols="2" align="right" style="display: flex; gap: 10px">
              <VBtn @click="zoneAddDialog = false"> ألغاء </VBtn>
  ...
  methods: {
    getAllPillars() {
      for (let each of this.data) {
        this.allPillars.push(...each.columns);
      }
    },
    paths(arr) {
      return arr.map((e) => {
    ...
    getZone() {
      if (this.addfiber.pillars.length > 0) {
        ...
        console.log(this.addfiber.pillars.length);
      } else {
        console.log("this is the else", this.addfiber.pillars);
    ...
    addZoneDialogRules() {
      console.log(this.addfiber.zonePoints);
      console.log(this.whichDialog);
    ...
    getGeoPoint(e) {
      if (this.whichDialog === "edit") {
        ...
        console.log(ref(this.$refs).value.pillar[0].marker);
      }
    },
    undo() {
